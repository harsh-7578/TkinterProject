...
def clicksubmit():
    ...
    if(gen==1):
        ...
        gender+="Female"    
    c.execute("""INSERT INTO StudentData(Name,Roll,Gender,Maths,Physics,Chemistry) VALUES(?,?,?,?,?,?)""",(studentname,rollnum,gender,math_marks,phy_marks,chem_marks))
...
con=sqlite3.connect('studentrecords.db')
c=con.cursor()
c.execute("""CREATE TABLE IF NOT EXISTS StudentData(Name TEXT,Roll NUMBER,Gender TEXT,Maths NUMBER,Physics NUMBER,Chemistry NUMBER)""")
# Creating Labels
...
def mathres():
    ...
    for row in rows:
        ...
        roll=row[1]
        math=row[2]
        lab1=Label(newwind,font=("Helvetica",10),fg="purple",bg="lightpink",highlightcolor="purple",text=s_name).place(x=x1,y=y1)
        lab2=Label(newwind,font=("Helvetica",10),fg="purple",bg="lightpink",highlightcolor="purple",text=str(roll)).place(x=x1+150,y=y1)
